...
linksGitHubPages.forEach((linkData, index) => {
    if (index < gitHubPagesContainers.length) {
        ...
        targetContainer.appendChild(
            createGitLinkPages(linkData.url, linkData.text)
        );
    }
});
...
const gitLink = createIconLinkButton({
    url: linksGitHub.url,
    text: linksGitHub.text,
    pathD: gitHubPath
});

const gitLink = createIconLinkButton({
    url: linksGitHubPages.url,
    text: linksGitHubPages.text,
    pathD: gitHubPagesPath
});
